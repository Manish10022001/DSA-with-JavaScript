...
function removeDuplicates(num) {
  // will assume 2 pointer, one will iterate through array to find unique element and
  //2nd will maintain the position to place the unique element
  let x = 0; //2nd pointer, first index as 0
  for (let i = 0; i < num.length; i++) {
    //1st pointer
    //if find the unique element;
    if (num[i] > num[x]) {
      //increase x i.e index to store new unique element;
      x = x + 1;
      num[x] = num[i];
    }
  }
